<template>
    ...
    <div class="grid lg:grid-cols-2 gap-6">
      ...
      <div class="border focus-within:border-blue-500 focus-within:text-blue-500 transition-all duration-500 relative rounded p-1">
        <div class="-mt-4 absolute tracking-wider px-1 uppercase text-xs">
          <p>
            <label for="name" class="bg-white text-gray-600 px-1">Estatus *</label>
          </p>
        </div>
        <p>
          <select v-model="estatus" autocomplete="false" tabindex="0" class="py-1 px-1 outline-none block h-full w-full">
...
export default ({
    mounted() {
        ...
        this.currentEstudianteId = window.location.href.substring(window.location.href.lastIndexOf('/') + 1);

        axios.get(process.env.MIX_BASEURL + '/estudiante/' + this.currentEstudianteId).then(res=> {
            this.nombre = res.data.nombre;
            this.apellido = res.data.apellido;
            this.email = res.data.email;
            this.carrera = res.data.carrera.id;
    ...
    methods: {
        store() {
            axios.post(process.env.MIX_BASEURL + '/estudiante/' + this.currentEstudianteId, {
                nombre: this.nombre,
